...
const serviceImage = computed(() => {
  ...
  // Fallback for German terms or mapping issues
...
const handleImageError = (e) => {
  ...
  if (e.target.src !== tradeImages.OTHER) {
    console.warn(`Could not load image for trade: ${safeService.value.trade}. Loading fallback.`);
...
<template>
  <div v-if="service && service.id" class="job-card">
    ...
    <div class="info-container" @click="goToDetail">
      ...
      <span class="trade-info" v-if="service.trade">
        {{ translatedTrade }}
      </span>
...
.job-card:has(.card-footer-actions) {
